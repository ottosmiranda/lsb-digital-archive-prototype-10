...
import { getBrowserSpecificEmbedUrl, detectBrowserCapabilities } from "@/utils/browserUtils";
...
const SpotifyPlayerSection = ({
  ...
  // Enhanced embed URL with browser-specific optimizations
  const optimizedEmbedUrl = getBrowserSpecificEmbedUrl(
    selectedEpisode?.embedUrl || embedUrl, 
    browserCapabilities
  );
  ...
  const handleIframeLoad = () => {
    ...
    // Only do autoplay detection for Spotify embed on non-mobile
    if (selectedEpisode?.isSpotifyEpisode && iframeRef.current && !browserCapabilities.isMobile) {
  ...
  const currentEmbedUrl = optimizedEmbedUrl;
  ...
  return (
    <div className={`bg-white border rounded-xl shadow-sm overflow-hidden ${playingFirst ? "ring-2 ring-green-500" : ""}`}>
      ...
      <div className="p-4 relative">
        ...
        {/* Embed error overlay */}
        {embedError && (
          <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-white/90 p-4 rounded-lg border border-orange-200">
            <div className="text-orange-700 text-base font-semibold mb-2 flex items-center gap-2">
              <AlertCircle className="h-5 w-5" /> Player indisponível
            </div>
            <div className="text-gray-700 text-sm mb-4 text-center max-w-xs">
              Não foi possível carregar o player do Spotify no seu navegador ({browserCapabilities.browserName}).
            </div>
          </div>
        )}
        ...
        {/* Direct iframe with error handling */}
        ...
        {/* Enhanced fallback for generated episodes */}
        {selectedEpisode && !selectedEpisode.isSpotifyEpisode && !selectedEpisode.embedUrl && (
          <div className="flex items-center justify-center h-[352px] bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
            <div className="text-center text-gray-500">
              <Play className="h-12 w-12 mx-auto mb-2 text-gray-400" />
              <p className="font-medium">Episódio de Exemplo</p>
              <p className="text-sm">Configure o Spotify para ouvir episódios reais</p>
              <p className="text-xs mt-2 text-gray-400">
                Navegador: {browserCapabilities.browserName}
                {browserCapabilities.isMobile && " (Mobile)"}
              </p>
            </div>
          </div>
        )}
