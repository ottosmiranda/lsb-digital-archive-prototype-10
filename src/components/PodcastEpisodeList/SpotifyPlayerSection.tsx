...
const SpotifyPlayerSection = ({
  ...
  const displayDescription = selectedEpisode 
    ? selectedEpisode.description 
    : "Ouça o episódio mais recente diretamente no Spotify";
  
  const displayDate = selectedEpisode 
    ? formatDate(selectedEpisode.date) 
    : new Date().toLocaleDateString("pt-BR", { day: "2-digit", month: "short", year: "numeric" }).replace(/\./g, "");
  
  const displayDuration = selectedEpisode 
    ...
    <div className={`bg-white border rounded-xl shadow-sm overflow-hidden ${playingFirst ? "ring-2 ring-green-500" : ""}`}>
      <div className="p-4 border-b">
        <div className="flex gap-2 items-center mb-2">
          ...
          {(!selectedEpisode || selectedEpisode.id === "1") && (
            <Badge className="bg-green-600 text-white ml-1">NOVO</Badge>
          )}
          {selectedEpisode && (
            <Badge className="bg-blue-600 text-white ml-1">REPRODUZINDO</Badge>
          )}
        </div>
