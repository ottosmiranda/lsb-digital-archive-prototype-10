    ...
    <section id="destaques-semana" className="py-8 md:py-16 lg:py-24 bg-slate-50">
      <div className="lsb-container">
        <div className="lsb-content">
          ...
          {highlights.length === 0 ? <div className="text-center my-8 md:my-12 text-lg text-gray-400">Nenhum destaque encontrado.</div> : <div className="relative">
            ...
            containScroll: "trimSnaps"
                ...
                <CarouselContent className="-ml-2 md:-ml-4">
                  {highlights.map((item, index) => <CarouselItem key={`${item.type}-${item.id}`} className="pl-2 md:pl-4 basis-full xs:basis-1/2 md:basis-1/3 lg:basis-1/4">
                      <Card className="group hover-lift animate-fade-in cursor-pointer border-0 shadow-lg hover:shadow-xl transition-all duration-300 h-full" style={{
                  animationDelay: `${index * 0.1}s`
                }} onClick={() => navigate(`/recurso/${item.id}`)}>
                        <CardContent className="p-0 h-full flex flex-col">
                          <div className="relative overflow-hidden rounded-t-lg">
                            {shouldShowImage(item.thumbnail, item.type) ? <img src={item.thumbnail} alt={item.title} className="w-full h-40 md:h-48 object-cover transition-transform duration-300 group-hover:scale-105" onError={e => {
                        console.error('🖼️ THUMBNAIL ERROR:', {
                      ...
                      }} /> : <ThumbnailPlaceholder type={item.type} className="w-full h-40 md:h-48" size="large" />}
                          ...
                          <div className="p-2 md:p-3 flex-1 flex flex-col">
                            ...
                            <WipeButton className="w-full text-xs py-2 px-3 text-sm" onClick={e => {
                        e.stopPropagation();
