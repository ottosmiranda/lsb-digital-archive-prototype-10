...
const SearchWelcomeState = ({ onQuickSearch }: SearchWelcomeStateProps) => {
  const navigate = useNavigate();

  const popularTopics = [
    'Educação', 'Cultura Surda', 'Inclusão', 'Tecnologia', 
    'Literatura', 'História', 'Comunicação', 'Direitos'
  ];
    ...
    <div className="text-center py-12 max-w-4xl mx-auto">
      ...
      <div className="mb-12">
        ...
        <p className="text-lg text-gray-600 mb-6 max-w-2xl mx-auto">
          Explore nossa coleção de recursos educacionais em Libras. 
          Use a busca para encontrar exatamente o que você precisa ou navegue pelos nossos tópicos populares.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        {contentTypes.map((type, index) => (
          <Card key={index} className={`${type.color} border-2 hover-lift`}>
        ...
        <div className="flex flex-wrap justify-center gap-3">
          {popularTopics.map((topic, index) => (
            <Button
              ...
              className="border-lsb-primary text-lsb-primary hover:bg-lsb-primary hover:text-white transition-colors"
            >
              {topic}
            </Button>
          ))}
